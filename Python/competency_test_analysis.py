...
def get_competency_result(member_id):

    loadfile = None
    for subdir, dirs, files in os.walk(competency_dir):
      for i, file in enumerate(files):
          if f'Member-{member_id}_' in file:
              loadfile = file
              break
    if loadfile is None:
        print(f"No competency file found for member {member_id}")
        return 0
    ...
    filepath = subdir + os.sep + loadfile
    ...
    return score.at['aggregate_time', 'timeSpent']
...
#     print(get_competency_result(26))
